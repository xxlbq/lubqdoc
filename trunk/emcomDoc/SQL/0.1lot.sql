...
SELECT ORDER_ID,EXECUTION_ID,EXECUTION_AMOUNT ,COMMISSION,COMMISSION_TYPE FROM JHF_ALIVE_EXECUTION WHERE ORDER_ID IN (
    SELECT ORDER_ID FROM JHF_ALIVE_ORDER WHERE ORDER_BIND_ID = '2009112023516954'
);
...
SELECT ORDER_ID,CONTRACT_ID,OPEN_COMMISSION,SETTLEMENT_COMMISSION ,SETTLED_TYPE,FORCE_RELATION_ID ,FORCE_RELATION_FLAG FROM JHF_ALIVE_CONTRACT WHERE ORDER_ID IN (
    SELECT TOP_ORDER_ID FROM JHF_ALIVE_ORDER WHERE ORDER_BIND_ID = '2009112023516954'
);
...
SELECT *  FROM JHF_COMMISSION_CONFIG         WHERE  CURRENCY_PAIR = 'EUR/JPY' AND GROUP_ID = 'DEFAULT' ;

SELECT *  FROM JHF_COMMISSION_SPECIAL_CONFIG WHERE  CURRENCY_PAIR = 'EUR/JPY' AND GROUP_ID = 'DEFAULT' ;
...
SELECT PRODUCT_ID,LEVERAGE_LEVEL_ID,CURRENCY_PAIR,LOSSCUT_COMMISSION FROM JHF_PRODUCT WHERE CURRENCY_PAIR = 'EUR/JPY' AND LEVERAGE_LEVEL_ID = 100
...
UPDATE JHF_PRODUCT  SET LOSSCUT_COMMISSION = 50 WHERE PRODUCT_ID='A002';
...
SELECT PRODUCT_ID,LEVERAGE_LEVEL_ID,CURRENCY_PAIR, TRADE_DEPOSIT FROM JHF_PRODUCT WHERE CURRENCY_PAIR = 'EUR/JPY' AND LEVERAGE_LEVEL_ID = 100





SELECT * FROM JHF_HEDGE_CUSTTRADE WHERE CURRENCY_PAIR = 'EUR/JPY' AND AMOUNT_NOT_HEDGED > 0 AND TYPE IN (3,2);

DELETE FROM JHF_HEDGE_CUSTTRADE WHERE AMOUNT_NOT_HEDGED > 0 

SELECT * FROM JHF_APPLICATION_DATE;

SELECT STOP_TIME_BEFORE_EOD FROM JHF_STATUS_CONTRL WHERE CURRENCY_PAIR ='EUR/JPY';

UPDATE JHF_STATUS_CONTRL SET STOP_TIME_BEFORE_EOD =10 WHERE CURRENCY_PAIR ='EUR/JPY';
UPDATE JHF_APPLICATION_DATE SET NEXT_EOD_START_TIME=DATE_ADD(CURRENT_TIMESTAMP, interval 5 HOUR) WHERE DATE_KEY = 'F'
