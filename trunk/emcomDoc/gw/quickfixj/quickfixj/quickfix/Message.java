...
public class Message extends FieldMap {
    ...
    void parse(String messageData, DataDictionary sessionDataDictionary,
        ...
        if (this.messageData.indexOf("FXCM") > 0 ){
        	if (this.messageData.indexOf("35=8") >0 )
        		this.messageData = this.messageData.replaceAll("1094=0", "");
        	else
        		this.messageData = this.messageData.replaceAll("537=1", "");
        	
        }
        
        if(this.messageData.indexOf("35=AE") >0 ){
        	
        	int begin = this.messageData.indexOf("1117", 0);
        	int end   = this.messageData.lastIndexOf("1117");
        	
        	String  rep =this.messageData.substring(begin, end);
        	
        	this.messageData = this.messageData.replaceAll(rep, "");
//        	System.out.println("messageData new:"+this.messageData );
        }
