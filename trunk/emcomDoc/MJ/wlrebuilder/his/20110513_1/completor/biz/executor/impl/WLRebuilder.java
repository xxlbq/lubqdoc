...
import cn.bestwiz.jhf.core.dao.bean.main.JhfWlPositionAutoRebuild;
import cn.bestwiz.jhf.core.dao.bean.main.JhfWlPositionAutoRebuildId;
	...
	public void rebuild(RebuildTask rebuildTask){
		try {
			...
			if(rebuildTotal == null || BigDecimal.ZERO.compareTo(rebuildTotal) == 0){
				JhfWlPositionAutoRebuild par = makePositionAutoRebuild(rebuildTask,BigDecimal.ZERO);
				service.posRebuild(null,null,null ,null,par);
				LOGGER.info("no position need rebuild ,write JhfWlPositionAutoRebuild  customerId : {} ,currencyPair : {} ,frontDate : {} ,amount : {}" 
						,new Object[]{rebuildTask.getCustomerId(),rebuildTask.getCurrencyPair(),rebuildTask.getApplicationDate().getId().getFrontDate(),BigDecimal.ZERO});
			}else{
				...
				JhfWlPositionAutoRebuild par = makePositionAutoRebuild(rebuildTask,Amount);
				service.posRebuild(rebuildSettleOrder,jspiSettle,rebuildOpenOrder ,jspiOpen,par);

				LOGGER.info("rebuild done, customerId : {} ,currencyPair : {} ,frontDate : {} ,amount : {}" 
						,new Object[]{rebuildTask.getCustomerId(),rebuildTask.getCurrencyPair(),rebuildTask.getApplicationDate().getId().getFrontDate(),Amount});
			}
...
//			LOGGER.info("order response was successfully sent, clientId: {}, wlOrderId: {}, orderStatus: {}, failReason: {}, executionPrice: {}, totalDelay: {}", new Object[]{order.getClientId(), order.getWlOrderId(), order.getOrderStatus(), order.getFailReason(), response.getExecutionPrice(), (System.currentTimeMillis() - timestamp)});
	...
	private JhfWlPositionAutoRebuild makePositionAutoRebuild(RebuildTask rebuildTask ,BigDecimal amount){
		JhfWlPositionAutoRebuild par = new JhfWlPositionAutoRebuild();
		
		JhfWlPositionAutoRebuildId parId = new JhfWlPositionAutoRebuildId();
		parId.setCustomerId(rebuildTask.getCustomerId());
		parId.setFrontDate(rebuildTask.getApplicationDate().getId().getFrontDate());
		parId.setCurrencyPair(rebuildTask.getCurrencyPair());
		
		par.setId(parId);
		par.setActiveFlag(new BigDecimal( BoolEnum.BOOL_YES_ENUM.getValue()));
		par.setAmount(amount);
		par.setInputDate(new Date(rebuildTask.getCurrentTime().getTime()));
		par.setUpdateDate(new Date(rebuildTask.getCurrentTime().getTime()));
		return par;
	}
...
//			BeanUtils.copyProperties(open, rebuildSettleOrder);
	...
	private JhfWlOrder toRebuildOrder(BigDecimal amount,
		...
		wlOrder.setOrderRequestTime(rebuildTask.getCurrentTime());
		wlOrder.setOrderRequestTimeLong(rebuildTask.getCurrentTime().getTime());
		wlOrder.setOrderAcceptTime(rebuildTask.getCurrentTime());
		wlOrder.setOrderAcceptTimeLong(rebuildTask.getCurrentTime().getTime());
...
//		wlOrder.setExecutionPrice(null);
		...
		wlOrder.setInputDate(new Date(rebuildTask.getCurrentTime().getTime()));
		wlOrder.setInputStaffId(rebuildTask.getStaffId());
		wlOrder.setUpdateDate(new Date(rebuildTask.getCurrentTime().getTime()));
		wlOrder.setUpdateStaffId(rebuildTask.getStaffId());
...
//		return date.getFrontEndDatetime().getTime() - info.getTradableEndInterval().intValue() + gap;
	...
	public WLSpotRateInfo findCloseRate(AutoBuildInfo info, String currencyPair,JhfApplicationDate date) throws Exception {
		...
		rateInfo.setBidRate((BigDecimal)rate.get("BID_RATE"));
