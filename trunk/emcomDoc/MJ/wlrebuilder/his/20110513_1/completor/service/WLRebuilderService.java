...
public class WLRebuilderService {
	...
	public void posRebuild(JhfWlOrder rebuildSettleOrder,
			...
			JhfSysPositionInsert jspiOpen ,JhfWlPositionAutoRebuild par) throws Exception {
		try {
			DbSessionFactory.beginTransaction(DbSessionFactory.MAIN);
			if(rebuildSettleOrder != null ) wlCompletorDao.saveWLOrder(rebuildSettleOrder);
			if(jspiSettle != null ) wlCompletorDao.saveJhfSysPositionInsert(jspiSettle);
			if(rebuildOpenOrder != null ) wlCompletorDao.saveWLOrder(rebuildOpenOrder);
			if(jspiOpen != null ) wlCompletorDao.saveJhfSysPositionInsert(jspiOpen);
			if(par != null ) wlCompletorDao.saveJhfWlPositionAutoRebuild(par);
			DbSessionFactory.commitTransaction(DbSessionFactory.MAIN);
		} catch (Exception e){
			DbSessionFactory.rollbackTransaction(DbSessionFactory.MAIN);
			throw e;
