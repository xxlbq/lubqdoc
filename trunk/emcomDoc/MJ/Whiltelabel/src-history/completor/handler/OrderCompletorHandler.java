...
//		aliveOrder.setChangeReason(new BigDecimal(ChangeReasonEnum.ORDER_REASON_REVERSE_ENUM.getValue()) );
	...
	private List<JhfAliveContract> queryPosList(String customerId,String ccy, BigDecimal side) {
		
		return ((WLCompletorService)ws).queryContract(customerId, ccy,side);
	}
...
//		jwb.setUpdateDate(Date);
	...
	private cn.bestwiz.jhf.core.dao.bean.main.JhfAliveOrder createJhfAliveOrder(
			JhfWlOrder jhfWlOrder, BigDecimal amount) throws IdGenerateException, CoreException {
		String productId = ((WLCompletorService)ws).queryProductId(jhfWlOrder.getCustomerId(),jhfWlOrder.getCurrencyPair());
		log.info(" product Id :{}",productId);
		return OrderInfoFactory.getInstance().converToJhfAliveOrder(jhfWlOrder,productId,amount);
	}
...
//		log.info("JhfAliveOrder OrderAmount:{}",jhfAliveOrder.getOrderAmount());
		...
		dao.saveJhfAliveOrder(jhfAliveOrder);
...
//		log.info("settledate:"+settledate);
		...
		dao.saveWlOrderBind(jhfWlOrderBind);
